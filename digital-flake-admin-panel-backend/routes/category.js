            ...
            .then((data_count) => {
                if (data_count == 0) {
                    category_model.aggregate([{
                        ...
                        category_model({ id: id, name: req.body.name, description: req.body.description, isActive: req.body.isActive, count: c, created_at: new Date() }).save().then((data_save) => {
    ...
    try {
        category_model.find({ name: { $regex: '^' + (req.body.name).trim() + '$', $options: 'i' } }).then((duplication_data) => {
            if (duplication_data.length == 1 && duplication_data[0].id == req.body.id) {
                category_model.updateOne({ id: (req.body.id).trim() }, { $set: { name: req.body.name, description: req.body.description, isActive: req.body.isActive, update_at: new Date() } }).then((update_data) => {
...
router.get('/findAll', (req, resp) => {
    ...
    try {
        category_model.find({ isActive: 1 }).then((find_data) => {
